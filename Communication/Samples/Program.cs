...
namespace Samples
{
    ...
    public class Driver
    {
        private const string PrimarySerialPortName = "COM6";
        ...
        public static async Task StartModbusSerialRtuSlaveNetwork(CancellationToken cancellationToken)
        {
            using (SerialPort slavePort = new SerialPort(PrimarySerialPortName))
            {
                ...
                IModbusSlave slave1 = factory.CreateSlave(10, acTechDataStore);
        ...
        public static void ReadWrite32BitValue()
        {
            using (SerialPort port = new SerialPort(PrimarySerialPortName))
            {
                ...
                Console.WriteLine($"Read 32 bit value {value} from slave {slaveId} starting at address {startAddress}");
            }
        ...
        public static float Read32BitFloat(IModbusSerialMaster master, byte slaveId, ushort startAddress)
        {
            ushort[] registers = master.ReadHoldingRegisters(slaveId, startAddress, 2);
            float value = ModbusUtility.GetSingle(registers[1], registers[0]);
            return value;
        }
        ...
        public static void ReadLoop()
        {
            using (SerialPort port = new SerialPort(PrimarySerialPortName))
            {
                ...
                byte slaveId = 10;
                ushort voltageAddressA_N = 3028;
                ushort voltageAddressB_N = 3030;
                ushort voltageAddressC_N = 3032;
                ushort voltageAddressL_N_Agv = 3036;

                ushort currentAddressA = 3000;
                ushort currentAddressB = 3002;
                ushort currentAddressC = 3004;
                ushort currentAddress_Agv = 3010;

                ushort TotalActivePowerAddress = 3060;
                ushort TotalReactivePowerAddress = 3068;
                ushort TotalApparentPowerAddress = 3076;


                ushort frequencyAddress = 3038;
                ushort TotalPowerFactorAddress = 3084;
                ...
                while (true)
                {
                    try
                    {
                        float voltageAN = Read32BitFloat(master, slaveId, voltageAddressA_N);
                        float voltageBN = Read32BitFloat(master, slaveId, voltageAddressB_N);
                        float voltageCN = Read32BitFloat(master, slaveId, voltageAddressC_N);
                        float voltageAgv = Read32BitFloat(master, slaveId, voltageAddressL_N_Agv);
                        float currentA = Read32BitFloat(master, slaveId, currentAddressA);
                        float currentB = Read32BitFloat(master, slaveId, currentAddressB);
                        float currentC = Read32BitFloat(master, slaveId, currentAddressC);
                        float currentAgv = Read32BitFloat(master, slaveId, currentAddress_Agv);
                        float totalActivePower = Read32BitFloat(master, slaveId, TotalActivePowerAddress);
                        float totalReactivePower = Read32BitFloat(master, slaveId, TotalReactivePowerAddress);
                        float totalApparentPower = Read32BitFloat(master, slaveId, TotalApparentPowerAddress);
                        float frequency = Read32BitFloat(master, slaveId, frequencyAddress);
                        float totalPowerFactor = Read32BitFloat(master, slaveId, TotalPowerFactorAddress);

                        Console.WriteLine($"Voltage: {voltageAN:F2} V, {voltageBN:F2} V, {voltageCN:F2} V, {voltageAgv:F2} V");
                        Console.WriteLine($"Current: {currentA:F2} A, {currentB:F2} A, {currentC:F2} A, {currentAgv:F2} A");
                        Console.WriteLine($"Power: {totalActivePower:F2} kW, {totalReactivePower:F2} kVAR, {totalApparentPower:F2} kVA");
                        Console.WriteLine($"Frequency: {frequency:F2} Hz, Power Factor: {totalPowerFactor:F2}");
                    }
                    catch (Exception ex)
                    {
